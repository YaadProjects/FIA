...
public class GroupChatImpl extends QBMessageListenerImpl<QBGroupChat> implements Chat, QBMessageSentListener<QBGroupChat>, QBIsTypingListener<QBGroupChat> {
...
//                        Toast.makeText(chatActivity, "Join successful", Toast.LENGTH_LONG).show();
    ...
    @Override
    public void updateLastMessage(QBDialog qbDialog, String lastMessage) {
        qbDialog.setLastMessage(lastMessage);
        QBRequestUpdateBuilder requestBuilder = new QBRequestUpdateBuilder();
        groupChatManager.updateDialog(qbDialog, requestBuilder, null);
    }
    ...
    public void processMessage(QBGroupChat groupChat, QBChatMessage chatMessage) {
        ...
        if (chatActivity == null)
            return;
        chatActivity.onReceivedMessage(chatMessage);
    }
    ...
    public void processUserIsTyping(QBGroupChat qbGroupChat, Integer userId) {
        if (chatActivity == null)
            return;
        chatActivity.onUserIsTyping(userId);
    }
    ...
    public void processUserStopTyping(QBGroupChat qbGroupChat, Integer userId) {
        if (chatActivity == null)
            return;
        chatActivity.onUserStopTyping(userId);
    }
}
